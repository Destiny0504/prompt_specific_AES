...
def main(args):
    ...
    for ckpt in range(
        ...
        try:
            ...
            pass
        exit()
        dataset = AsapMixedDatasetForRegression(args.dataset_path, train=False)
        ...
        with torch.no_grad():
            for data, target in test_data:
                ...
                all_target += target
        # print(all_predict)
        all_predict = [round(predict * dataset.label_scaler[args.dataset_path[-4:-3]] + dataset.label_min[args.dataset_path[-4:-3]]) for predict in all_predict]
        ...
        print(score)
        writer.add_scalar(f"QWK prompt {args.dataset_path[-4:-3]}", score, ckpt)
